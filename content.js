...
setInterval(function () {
    ...
    // document.getElementById("wool").innerHTML = 1;
    chrome.runtime.sendMessage({ message: "data_from_content_script", data: 1 });
...
function identifyAction(text, html) {
    ...
    if (text.includes('got')) {
        ...
        player = text.split(" ")[0]
        // console.log(player, 'TRADE with Bank', text)
        ...
        const imgTagsGaveBank = textBetween.match(/<img[^>]+>/g) || [];
        const gaveBankCards = imgTagsBetween.map(img => img.match(/alt="([^"]*)"/)[1]);
        ...
        const imgTagsFromBank = textAfterTook.match(/<img[^>]+>/g) || [];
        const tookFromBankCards = imgTagsAfterTook.map(img => img.match(/alt="([^"]*)"/)[1]);

        // Output the number of images and their alt attributes
        // console.log('Number of images between "gave bank" and "and took":', imgTagsGaveBank.length);
        // console.log('Alt attributes of images between "gave bank" and "and took":', gaveBankCards);
        // console.log('Number of images after "and took":', imgTagsFromBank.length);
        // console.log('Alt attributes of images after "and took":', tookFromBankCards);

        if(player in players){
            gaveBankCards.forEach(element => {
                players[player][element] -= 1
            });
            tookFromBankCards.forEach(element => {
                players[player][element] += 1
            });
        }
        ...
        //pass
    } else if (text.includes('traded')) {
        console.log('TRADE', text, html)
        //pass
    } else {
        isValidAction = false
    }
